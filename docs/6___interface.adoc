...
In Modelica this is the case for replaceable classes (see <<redeclaration>>) and for `inner`/`outer` elements (see <<instance-hierarchy-name-lookup-of-inner-declarations>>).
...
[definition]
.Class type or inheritance interface
...
[definition]
.Subtype or compatible interface
...
[definition]
.Function subtype or function compatible interface
...
image::media/subtype.svg[]

The type `R` can be defined as the set of record values containing `x` and `b`.
The subtype `R2` is the subset of values that all contain `x`, `b`, and `y`.
...
[definition]
.Plug-compatibility (= restricted subtyping)
...
and for array concatenation in <<array-concatenation>>).
