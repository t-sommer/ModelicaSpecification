...
.Example: Non-associative exponentiation and array range operator
[source,modelica]
----
...
For example, `1 + -1 + 1` is an invalid expression, whereas both `1 + (-1) + 1` and `-1 + 1 + 1` are fine.

.Example: Unary minus and plus in Modelica vs Mathematica/MATLAB
[source,modelica]
----
...
.Syntax rules for arithmetic operators
[source,grammar]
----
...
.Syntax rules for relational and logical operators
[source,grammar]
----
logical-expression :
...
* For operands of type `String`, `str1 op str2` is defined in terms of the C function `strcmp(str1, str2) op 0`.
...
Concatenation of strings is denoted by the `+` operator in Modelica.
...
The two expressions, `expression2` and `expression3`, must be type compatible expressions giving the type of the if-expression.
...
* Mathematical functions and conversion operators, see <<numeric-functions-and-conversion-functions>> below.
...
Except for the `min`, `max` and the `String` conversion operator they are vectorizable according to <<scalar-functions-applied-to-array-arguments>>.
...
Additional non-event generating mathematical functions are described in <<built-in-mathematical-functions-and-external-built-in-functions>>, whereas the event-triggering mathematical functions are described in <<event-triggering-mathematical-functions>>.
...
.Function: Integer (conversion from enumeration)
[source,modelica]
----
Integer(e)
----
...
.Function: EnumTypeName (conversion from Integer)
[source,modelica]
----
EnumTypeName(i)
----
...
.Function: String (conversion to String)
[source,modelica]
----
String(b, <options>)
String(i, <options>)
...
The first argument may be a Boolean b, an Integer i, a Real r, or an enumeration value e.
...
The standard type coercion shall not be applied for the first argument of String.
...
The operators listed below trigger events if used outside of a when-clause and outside of a clocked discrete-time partition.
...
These expressions for div, ceil, floor, and integer are event generating expressions.
The event generating expression for mod(x,y) is floor(x/y), and for rem(x,y) it is div(x,y).
...
div(x, y)
----
Algebraic quotient x / y with any fractional part discarded (truncation toward zero).
...
|===
|Expression |Description |Details
|`der(expr)` |Time derivative |<<modelica:der>>
|`delay(expr, ...)` |Time delay |<<modelica:delay>>
|`cardinality(c)` |Number of occurrences in connect-equations |<<modelica:cardinality>>
|`homotopy(actual, simplified)` |Homotopy initialization |<<modelica:homotopy>>
|`semiLinear(x, k+, k-)` |Sign-dependent slope |<<modelica:semiLinear>>
|`inStream(v)` |Stream variable flow into component |<<modelica:inStream>>
|`actualStream(v)` |Actual value of stream variable |<<modelica:actualStream>>
|`spatialDistribution(...)` |Variable-speed transport |<<modelica:spatialDistribution>>
|`getInstanceName()` |Name of instance at call site |<<modelica:getInstanceName>>
|===

See the original document for detailed semantics and examples for each operator.

==== Event-Related Operators with Function Syntax

The operators listed below are event-related operators with function syntax.
The operators `noEvent`, `pre`, `edge`, and `change`, are vectorizable according to <<scalar-functions-applied-to-array-arguments>>.

[cols="1,2,2",options="header"]
|===
|Expression |Description |Details
|`initial()` |Predicate for the initialization phase |<<modelica:initial>>
|`terminal()` |Predicate for the end of a successful analysis |<<modelica:terminal>>
|`noEvent(expr)` |Evaluate expr without triggering events |<<modelica:noEvent>>
|`smooth(p, expr)` |Treat expr as p times continuously differentiable |<<modelica:smooth>>
|`sample(start, interval)` |Periodic triggering of events |<<modelica:event-sample>>
|`pre(y)` |Left limit y(t-) of variable y(t) |<<modelica:pre>>
|`edge(b)` |Expands into (b and not pre(b)) |<<modelica:edge>>
|`change(v)` |Expands into (v <> pre(v)) |<<modelica:change>>
|`reinit(x, expr)` |Reinitialize x with expr |<<modelica:reinit>>
|===
...
* For multiple return assignment `(x1, ..., xn) := expr`, all of `x1`, ..., `xn` must be declared to be at least as variable as `expr`.
* When determining whether an equation can contribute to solving for a variable `v` (for instance, when applying the perfect matching rule), the equation can only be considered contributing if the resulting solution would be at most as variable as `v`.

.Example: Variability constraints
[source,modelica]
----
model Constants
  parameter Real p1 = 1;
...
This is especially a concern for functions given as a short class.
This has additional implications for redeclarations.
The purity of the function does not influence the variability of the function call.
...
* Some function calls are constant expressions regardless of the arguments: `ndims(A)`.
...
* Some function calls are evaluable expressions even if the arguments are not: `cardinality(c)`, `size(A)`, `Connections.isRoot(A.R)`, `Connections.rooted(A.R)`.
...
* Evaluable expressions.
* Non-evaluable parameter variables.
...
* Some function calls are parameter expressions even if the arguments are not: `size(A, j)` where `j` is a parameter expression, if `A` is variable declared in a non-function class.
...
* Expressions in a clocked discrete-time partition.
...
For example, `time` is a continuous-time built-in variable and `time + 1` is a non-discrete-time expression.
