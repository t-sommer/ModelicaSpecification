...
Consider a connection set with _n_ connectors, and denote the mass flow rates `m_flow` by $\tilde{m}$.
...
image::media/fluidmix.svg[caption="Exemplary connection set with three connected components and a common mixing enthalpy."]
...
Equation (eq:D2a) is solved for $h_{\mathrm{mix}}$:
...
We establish this quantity using a dedicated operator `inStream(h_outflow,i) = h_mix` assuming that $\tilde{m}_i \geq 0$.
...
image::media/fluidmix3.svg[caption="Exemplary connection set with three connected components."]
...
In this case, `inStream` is continuous (contrary to $h_{\mathrm{mix}}$) and does not depend on flow rates.
...
image::media/fluidmix4.svg[caption="Example series connection of multiple models with stream connectors."]
...
Consider the case of $\tilde{m}_{1} < 0$ and all other mass flow rates positive (with the `min` attribute set accordingly).
...
In such a case, any value of $h_{\mathrm{mix}}$ fulfills (eq:D1), i.e., a unique mathematical solution does not exist.
