...
The stream connector part is primarily intended to exclude sensor-variables, see <<connection-of-3-stream-connectors-where-one-mass-flow-rate-is-identical-to-zero-n-3-and>>, but also excludes non-reversible outgoing flows.
====
...
[The reason for making the tool-dependent outline rudimentary is to encourage the model developer to
provide a proper icon. That visible-attributes are not regarded makes it possible to obtain an icon which
only shows connectors by adding a dummy primitive with visible = false.]

==== Extends-Clause
:id: extends-clause

Each `extends`-clause (and short class definition, as stated in <<annotations-for-graphical-objects>>) may have layer specific annotations which describe the rendering of the base class' icon and diagram layers in the derived class.
...
+
.Example
If `par = "Modelica.Blocks.Types.Enumeration.Periodic"`, then %_par_ should be displayed as _Periodic_.
+
...
.Example
A button can be represented by a rectangle changing color depending on a `Boolean` variable `on` and toggles the variable when the rectangle is clicked on:
...
:id: annotations-for-the-graphical-user-interface
...
When specified and an `outer` component of the class does not have a corresponding `inner` component, the string message may be used as part of a diagnostic message (together with appropriate context), see <<instance-hierarchy-name-lookup-of-inner-declarations>>.
...
[#modelica:defaultConnectionStructurallyInconsistent]
...
.Example
When `group` is empty, a tool may place parameters in the group "Parameters", and place variables with `showStartAttribute = true` in the group "Start Attributes".
...
_Example:_ Assume the connection line in the resulting example in case <<connectorSizing:addVector>> is removed.
This results in:
+
[source,modelica]
----
...
If a new connection line is drawn to an inside connector with `connectorSizing` and case <<connectorSizing:addVector>> does not apply then, the parameter is incremented by one and the connection is performed for the new highest index.
...
If a connection line is deleted to an inside connector with `connectorSizing` and case <<connectorSizing:deleteVector>> does not apply then, then the (`connectorSizing`) parameter is decremented by one and all connections with index above the deleted connection index are also decremented by one.
_Example:_Assume there are 4 connections:
+
[source,modelica]
----
  Step step1(nIn=4);
...
=== Versions
:id: annotations-for-version-handling
...
These functions can be called with literal strings or arrays of literal strings and vectorize according to <<scalar-functions-applied-to-array-arguments>>.
...
:id: convertclassoldclassnewclass
...
:id: convertclassifoldclass-oldelement-whenvalue-newclass
...
:id: convertelementoldclassoldnamenewname
...
:id: convertmessageoldclass-failed-message
...
:id: convertmessageoldclass-failed-message2
...
If the parameter had no impact on the model it can be removed using `convertModifiers`, see <<convertmodifiers>>.
====
...
. `Access.icon`
The class can be instantiated and public parameter, constant, input, output variables as well as public connectors can be accessed, as well as the `Icon` annotation, as defined in <<annotations-for-graphical-objects>> (the declared information of these elements can be shown).
Additionally, the class name and its description text can be accessed.
. `Access.documentation`
Same as `Access.icon` and additionally the `Documentation` annotation (as defined in <<annotations-for-documentation>>) can be accessed.
...
.Example
If the annotation is given on the top level of a package and at no other class in this package, then the `access` annotation holds for all classes in this package.
...
.Example
For instance a class with `Access.hide` should not be used in the diagram layer of a class with `Access.diagram`, and there should not be hyperlinks to classes with `Access.icon` (from classes with visible documentation).
...
.Example
With the same package `P` as in the previous example, consider the following invalid use outside of `P`:
...
External functions may also contain this annotation (with similar content), see <<annotations-for-external-libraries-and-include-files>>.
