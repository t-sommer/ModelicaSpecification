...
`S-CHAR` is any member of the Unicode character set (https://unicode.org; see <<mapping-package-class-structures-to-a-hierarchical-file-system>> for storing as UTF-8 on files) except double-quote `"`, and backslash `\`.
