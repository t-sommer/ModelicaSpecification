...
Identifiers are sequences of letters, digits, and other characters such as underscore, which are used for _naming_ various items in the language.
Certain combinations of letters are keywords represented as _reserved_ words in the Modelica grammar and are therefore not available as identifiers.
...
A name is an identifier with a certain interpretation or meaning.
...
A component reference is an expression containing a sequence of identifiers and indices.
...
.Example
A component reference: `Ele.Resistor.u[21].r`

==== Modelica Keywords

The following Modelica keywords are reserved words that cannot be used where `IDENT` is expected in the language grammar (see <<modelica-concrete-syntax>>):
...
Literals are unnamed constants used to build expressions, and have different forms depending on their type.
...
The minimal recommended range is that of IEEE double precision floating point numbers, for which the largest representable positive number is 1.7976931348623157×10^308 and the smallest positive number is 2.2250738585072014×10^-308.
...
The predefined operator symbols are formally defined on page <<lexical-conventions>> and summarized in the table of operators in <<operator-precedence-and-associativity>>.