...
The size of such a variable can be determined from its binding equation, or the size of any of its array attributes (see also <<flexible-array-sizes-and-resizing-of-arrays-in-functions>>).
The size cannot be determined from other equations or algorithms.

Upper and lower array dimension index bounds are described in <<array-dimension-lower-and-upper-index-bounds>>.
...
Regarding flexible array sizes and resizing of arrays in functions, see <<flexible-array-sizes-and-resizing-of-arrays-in-functions>>.
...
|`promote(A, n)` |Append dimensions of size 1 |<<modelica:promote>>
|===
...
Let `C = promote(A, n)`, with n_A = ndims(A), then ndims(C) = n, size(C, j) = size(A, j) for 1 <= j <= n_A, size(C, j) = 1 for n_A + 1 <= j <= n, C[i1, ..., i_nA, 1, ..., 1] = A[i1, ..., i_nA]
...
|===
|Expression |Description |Details
|`ndims(A)` |Number of dimensions |<<modelica:ndims>>
|`size(A, i)` |Size of single array dimension |<<modelica:size-of-dim>>
|`size(A)` |Sizes of all array dimensions |<<modelica:size-vector>>
|===
...
size(A, i)
----
Returns the size of dimension i of array expression A where 1 <= i <= ndims(A).
...
|===
|Expression |Description |Details
|`scalar(A)` |Extract only element |<<modelica:scalar>>
|`vector(A)` |Vector of all elements |<<modelica:vector>>
|`matrix(A)` |Two-dimensional array |<<modelica:matrix>>
|===
...
matrix(A)
----
Returns promote(A, 2) if A is a scalar or vector, and otherwise returns the elements of the first two dimensions as a matrix.
size(A, i) = 1 is required for 2 < i <= ndims(A).
...
|`identity(n)`              |Identity matrix                     |<<modelica:identity>>
|`diagonal(v)`              |Diagonal matrix                     |<<modelica:diagonal>>
|`zeros(n1, n2, n3, ...)`   |Array with all elements being 0     |<<modelica:zeros>>
|`ones(n1, n2, n3, ...)`    |Array with all elements being 1     |<<modelica:ones>>
|`fill(s, n1, n2, n3, ...)` |Array with all elements equal       |<<modelica:fill>>
|`linspace(x1, x2, n)`      |Vector with equally spaced elements |<<modelica:linspace>>
|===
...
ones(n1, n2, n3, ...)
----
Returns the n1 x n2 x n3 x ... Integer array with all elements equal to one (ni >= 0).
...
fill(s, n1, n2, n3, ...)
----
Returns the n1 x n2 x n3 x ... array with all elements equal to scalar or array expression s (ni >= 0).
...
The arguments x1 and x2 shall be numeric scalar expressions.
...
|===
|Expression |Description |Details
|`min(A)` |Least element of array |<<modelica:min-of-array>>
|`min(x, y)` |Least of two scalars |<<modelica:min-binary>>
|`min(... for ...)` |Reduction to least value |<<modelica:min-reduction>>
|`max(A)` |Greatest element of array |<<modelica:max-of-array>>
|`max(x, y)` |Greatest of two scalars |<<modelica:max-binary>>
|`max(... for ...)` |Reduction to greatest value |<<modelica:max-reduction>>
|`sum(A)` |Sum of scalar array elements |<<modelica:sum-of-array>>
|`sum(... for ...)` |Sum reduction |<<modelica:sum-reduction>>
|`product(A)` |Product of scalar array elements |<<modelica:product-of-array>>
|`product(... for ...)` |Product reduction |<<modelica:product-reduction>>
|===
...
|===
|Expression |Description |Details
|`transpose(A)` |Matrix transpose |<<modelica:transpose>>
|`outerProduct(x, y)` |Vector outer product |<<modelica:outerProduct>>
|`symmetric(A)` |Symmetric matrix, keeping upper part |<<modelica:symmetric>>
|`cross(x, y)` |Cross product |<<modelica:cross>>
|`skew(x)` |Skew symmetric matrix associated with vector |<<modelica:skew>>
|===
...
skew(x)
----
Returns the 3 x 3 skew symmetric matrix associated with a 3-vector, i.e., cross(x, y) = skew(x) * y.
...
* Arrays A, B, C, ... must be type compatible expressions (see <<type-compatible-expressions>>) giving the type of the elements of the result.
  ...
  Real and Integer subtypes can be mixed resulting in a Real result array where the Integer numbers have been transformed to Real numbers.
* k has to characterize an existing dimension, i.e., 1 <= k <= ndims(A) = ndims(B) = ndims(C); k shall be a parameter expression of Integer type.
...
----
R[i1, ..., ik, ..., in] = A[i1, ..., ik, ..., in] for 0 < ik <= size(A,k)
R[i1, ..., ik, ..., in] = B[i1, ..., ik - size(A,k), ..., in] for size(A,k) < ik <= size(A,k) + size(B,k)
R[i1, ..., ik, ..., in] = C[i1, ..., ik - size(A,k) - size(B,k), ..., in] for size(A,k) + size(B,k) < ik <= size(A,k) + size(B,k) + size(C,k)
...
----

where 1 <= ij <= size(R,j) for 1 <= j <= n.
...
An indexing operation is subject to upper and lower array dimension index bounds (see <<array-dimension-lower-and-upper-index-bounds>>).
...
If x is a vector, x[1] is a scalar, but the slice x[1:5] is a vector
...
See <<array-indexing>> regarding assignments to array variables with vector of subscripts.
...
|n x m matrix |n x m matrix |n x m matrix |`a[j, k] = b[j, k]`
|n x m x ... |n x m x ... |n x m x ... |`a[j, k, ...] = b[j, k, ...]`
|===
...
|n x m matrix |n x m matrix |n x m matrix |`c[j, k] := a[j, k] ± b[j, k]`
|n x m x ... |n x m x ... |n x m x ... |`c[j, k, ...] := a[j, k, ...] ± b[j, k, ...]`
|===
...
|Size of a |Size of b |Size of a .± b |Operation c := a .± b
|Scalar |Scalar |Scalar |`c := a ± b`
|Scalar |n x m x ... |n x m x ... |`c[j, k, ...] := a ± b[j, k, ...]`
|n x m x ... |Scalar |n x m x ... |`c[j, k, ...] := a[j, k, ...] ± b`
|n x m x ... |n x m x ... |n x m x ... |`c[j, k, ...] := a[j, k, ...] ± b[j, k, ...]`
|===
...
|Scalar |n x m matrix |n x m matrix |`c[j, k] := s * a[j, k]`
|Scalar |n x m x ... |n x m x ... |`c[j, k, ...] := s * a[j, k, ...]`
|===
...
|m-vector |m x n matrix |n-vector |`c[j] := sum_{k} a[k] * b[k, j]`
|l x m matrix |m-vector |l-vector |`c[i] := sum_{k} a[i, k] * b[k]`
|l x m matrix |m x n matrix |l x n matrix |`c[i, j] := sum_{k} a[i, k] * b[k, j]`
|===
...
|n x m matrix |Scalar |n x m matrix |`c[j, k] := a[j, k] / s`
|n x m x ... |Scalar |n x m x ... |`c[j, k, ...] := a[j, k, ...] / s`
|===
...
See <<scalar-functions-applied-to-array-arguments>>.
