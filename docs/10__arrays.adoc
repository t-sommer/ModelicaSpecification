...
|`C[n, m] x;`            |`C x[n, m];`            |2       |Matrix      |n &times; m matrix
...
|`C[1, 1] x;`    |`C x[1, 1];`    |2      |Matrix      |(1 &times; 1)-matrix, representing a scalar
|`C[n, 1] x;`    |`C x[n, 1];`    |2      |Matrix      |(n &times; 1)-matrix, representing a column
|`C[1, n] x;`    |`C x[1, n];`    |2      |Matrix      |(1 &times; n)-matrix, representing a row
|===
...
Let `C = promote(A, n)`, with `n~A~ = ndims(A)`, then `+ndims(C) = n+`, `+size(C, j) = size(A, j)+` for `1 &le; j &le; n~A+~`, `size(C, j) = 1` for `n~A~ + 1 &le; j &le; n, C[i1, ..., i~nA~, 1, ..., 1] = A[i1, ..., i~nA~]`
...
----
+
Returns the number of dimensions k of expression A, with k &ge; 0.
...
size(A, i)
----
Returns the size of dimension i of array expression A where 1 &le; i &le; ndims(A).
+
...
`size(A, i) = 1` is required for 1 &le; i &le; `ndims(A)`.
...
`size(A, i)` = 1 is required for 2 < i &le; `ndims(A)`.
...
+
Recursive definition: latexmath:[fill(s, n_{1}, n_{2}, n_{3}, \ldots) = fill(fill(s, n_{2}, n_{3}, \ldots), n_{1}); fill(s, n) = \{s, s, \ldots, s\}].
+
