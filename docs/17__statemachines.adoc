...
|`transition(from, to, condition, ...)` |State machine transition between states |<<transition>>
|`initialState(state)` |State machine initial state |<<initialState>>
|===
...
|===
|Expression |Description |Details
|`activeState(state)` |Predicate for active state |<<activeState>>
|`ticksInState()` |Ticks since activation |<<ticksInState>>
|`timeInState()` |Time since activation |<<timeInState>>
|===
...
image::media/hierarchical-statemachine.svg[]
